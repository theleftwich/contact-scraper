...
for url in urls:
    try:
	    ...
	    # zoom in to contact paragraph
        contact_p = soup.find('p',attrs={'class':'name'})
	    
	    # what if there's no contact box?
        if contact_p is None:
            contact_name = "No Contact Box!"
        else:	
            contact_name = (contact_p.find('span',attrs={'class':'fn'})).string
	
    except (mechanize.HTTPError,mechanize.URLError) as e:
        if isinstance(e,mechanize.HTTPError):
            contact_name = str(e.code)
        else:
            contact_name = str(e.reason.args)
